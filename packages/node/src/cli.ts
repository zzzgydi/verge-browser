...
function parseDimensionOption(
  ...
  if (typeof value === "string" && Number.isFinite(Number(value)))
    return Number(value);
  throw new VergeConfigError(
    `option \`${name} <${name.slice(2)}>\` value is invalid`,
  );
}
...
function createCli(
  ...
  cli
    ...
    .usage(
      "verge-browser sandbox <list|ls|create|get|update|pause|resume|rm|cdp|session|restart> [idOrAlias] [options]",
    )
    ...
    .option("--height <height>", "Viewport height in pixels", {
      type: ["number"],
    })
    ...
    .action(
      async (
        ...
        if (action === "create") {
          ...
          const metadata = parseJsonObjectOption(
            "--metadata",
            options.metadata,
          );
        ...
        if (action === "update") {
          ...
          if (!options.alias && !metadata) {
            throw new VergeConfigError(
              "missing update fields; pass --alias and/or --metadata",
            );
          }
  ...
  cli
    .command(
      "browser <action> <idOrAlias>",
      "Inspect or control the browser inside a sandbox",
    )
    ...
    .option(
      "--target-id <targetId>",
      "Optional CDP target id for page screenshots",
    )
    ...
    .action(
      async (
        action: string,
        idOrAlias: string,
        options: BrowserScreenshotOptions & BrowserActionsOptions,
      ) => {
        const client = clientFactory(toClientOptions(options));
        if (action === "screenshot") {
          ...
          if (options.output) {
            const target = await writeOutputFile(
              options.output,
              Buffer.from(screenshot.data_base64, "base64"),
            );
  ...
  cli
    .command(
      "files <action> <idOrAlias> [path]",
      "Manage sandbox workspace files",
    )
    .usage(
      "verge-browser files <list|read|write|upload|download|rm> <idOrAlias> [path] [options]",
    )
    ...
    .action(
      async (
        ...
        options: FileWriteOptions & FileDownloadOptions,
      ) => {
        const client = clientFactory(toClientOptions(options));
        if (action === "list") {
        ...
        if (action === "write") {
          ...
          return client.writeFile(idOrAlias, {
            path,
        ...
        if (action === "upload") {
          ...
          return client.uploadFile(idOrAlias, {
...
async function loadBrowserActionsPayload(
  options: BrowserActionsOptions,
): Promise<BrowserActionsPayload> {
  if (options.input) {
    return readJsonFile<BrowserActionsPayload>(
      options.input,
      "option `--input`",
    );
  }
  if (options.payload) {
    return parseJsonValueOption<BrowserActionsPayload>(
      "--payload",
      options.payload,
    );
  }
  throw new VergeConfigError(
    "missing browser actions payload; pass --input or --payload",
  );
}
...
function emitResult(result: unknown, jsonOutput: boolean, io: CliIo): void {
  ...
  io.stdout.log(formatPlainText(result));
}

function formatPlainText(value: unknown, indent = 0): string {
  ...
  if (typeof value === "string") {
    return formatStringValue(value, indent);
  }
  if (
    typeof value === "number" ||
  ...
  if (Array.isArray(value)) {
    ...
    return value
      .map((item) => {
        ...
        const rendered = formatPlainText(item, indent + 1).split("\n");
        const [first = "", ...rest] = rendered;
        return [
          `${prefix}- ${first}`,
          ...rest.map((line) => `${prefix}  ${line}`),
        ].join("\n");
      })
      .join("\n");
  }
  if (typeof value === "object") {
    ...
    return entries
      .map(([key, child]) => formatObjectEntry(key, child, indent))
      .join("\n");
  }
  return String(value);
}
...
function formatObjectEntry(
  ...
  const rendered = formatPlainText(value, indent + 1);
  return `${prefix}${key}:\n${rendered}`;
}

function formatStringValue(value: string, indent: number): string {
  if (!value.includes("\n")) return value;
  const prefix = "  ".repeat(indent);
  const block = value
    .split("\n")
...
function handleError(error: unknown, jsonOutput: boolean, io: CliIo): number {
  if (error instanceof VergeConfigError)
    return emitError(error.message, EXIT_CONFIG, jsonOutput, io);
  if (error instanceof VergeAuthError)
    return emitError(error.message, EXIT_AUTH, jsonOutput, io);
  if (error instanceof VergeNotFoundError)
    return emitError(error.message, EXIT_NOT_FOUND, jsonOutput, io);
  if (error instanceof VergeConflictError)
    return emitError(error.message, EXIT_CONFLICT, jsonOutput, io);
  if (error instanceof VergeValidationError)
    return emitError(error.message, EXIT_VALIDATION, jsonOutput, io);
  if (error instanceof VergeServerError)
    return emitError(error.message, EXIT_SERVER, jsonOutput, io);
...
export async function runCli({
  argv,
  clientFactory = (options) => new VergeClient(options),
  io = defaultIo,
...
const isMainModule = (): boolean => {
  ...
  return entry === "cli.js" || entry === "verge-browser" || entry === "verge-browser.cmd";
};
