...
- 主 API 通过 `POST /sandboxes/{sandbox_id}/vnc/tickets` 签发 ticket
- ticket 必须绑定 `sandbox_id`、访问路径 scope、过期时间、nonce
- ticket 支持显式消费策略：
- `one_time`：默认值，TTL 默认 60 秒，首个成功入口消费后失效
- `reusable`：允许重复使用，但仍受 TTL 限制
- `permanent`：允许重复使用且不过期，仅建议用于受控调试环境
- 页面 URL 格式：
...
5. 若 ticket 为 `one_time`，则在首个成功消费后立即失效，避免 URL 泄露重放
...
- 请求体支持 `mode` 和 `ttl_sec`
- 响应返回 `ticket`、`mode`、`ttl_sec`、`expires_at`
