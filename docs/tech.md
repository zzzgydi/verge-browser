...
V1 采用“一容器一会话一浏览器”模型。每个 sandbox 对应一个容器实例，容器内运行浏览器、VNC、shell、API 代理等所有组件。
...
浏览器、shell、文件接口都共享 `/workspace`，保证下载、上传、代码生成、文件处理形成闭环。
...
+-------------------------------+
        ...
        v                             v
+--------------------+      +----------------------+
| Browser APIs       |      | Shell / File APIs    |
| screenshot/actions |      | exec/ws/read/write   |
+--------------------+      +----------------------+
        |
        +-----------------------------+
        |
...
- tmux：持久会话管理
- Python pty / asyncio.subprocess：命令执行
- xterm.js：前端终端（可选）
...
5. websockify
6. supervisord
7. shell 执行所需的 pty / tmux 进程

说明：

- 这里的 shell 能力仍属于同一容器内的运行时组件，不建议在 V1 引入独立 sidecar，以免偏离“单容器单会话”原则。
...
- 优先支持 `argv: ["ls", "-la", "/workspace"]`
- `command` 作为兼容模式显式标记为“通过 shell 解释执行”
- 二者最多二选一，避免调用方误解转义规则

执行限制建议明确为：

- 默认最大执行时长 `30s`
- 默认 stdout/stderr 返回大小上限，例如 `1MB`
- 超时返回 408 或明确业务错误码
...
- 每个 session 绑定一个 tmux pane 或 pty
- WebSocket 负责输入输出流

### 13.4 Shell 安全限制

V1 可接受 sandbox 内 root 运行，但生产建议：
...
- `POST /sandboxes/{id}/shell/exec`
- `POST /sandboxes/{id}/shell/sessions`
- `WS /sandboxes/{id}/shell/sessions/{sid}/ws`
...
1. 编写 Dockerfile
2. 安装 Chromium + Xvfb + openbox + x11vnc + websockify + tmux + xdotool
3. 编写 supervisor 配置
...
### 20.6 阶段 F：Shell / Files

任务：

1. 实现 shell exec
2. 实现交互终端 ws
3. 实现 files list/read/write/upload/download
4. 完成路径安全检查
...
7. shell 执行成功
8. 文件上传下载成功
9. 多 tab 场景下 page screenshot 命中正确 target
10. hard restart 后恢复
11. 上传超限返回 413
...
3. CDP / VNC / shell WebSocket 并发连接数
...
7. shell exec 可以列出 `/workspace` 内容。
